...
test.describe('Conduit - Add article tests', () => {
  ...
  test('should navigate properly from main view to add article page', async ({ navbarComponent, mainView }) => {
    await mainView.goto();
    await navbarComponent.gotoAddArticlePage();
    
    await expect(mainView.page).toHaveURL('https://demo.realworld.show/editor');
  }); 
